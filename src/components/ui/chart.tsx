...
function useChart() {
  ...
  if (!ctx) throw new Error("useChart must be used inside <Chart.Root>")
  return ctx
}

// ── Root — solo contexto + estilo, sin layout forzado ─────────────────
  ...
  return (
    <ChartContext.Provider value={{ config }}>
      <div
        ...
        style={style}
        className={cn(
          "relative flex w-full justify-center text-xs",
          /* recharts overrides — themes via CSS variables */
...
// ── Style — inyecta CSS variables de color (se puede usar standalone) ──
...
// ── Layout helpers — wrappeás como quieras ────────────────────────────

type ChartWrapperProps = ComponentProps<"div"> & {
  /** Aspect ratio shortcut, ej: "video" | "square" | "auto" | "4/3" */
  aspect?: string
}

/** Wrapper opcional para controlar aspect ratio y padding del chart. */
const ChartFrame = ({
  className,
  aspect,
  style,
  ...
  <div
    className={cn(
      ...
      aspect && !["video", "square", "auto"].includes(aspect) && `aspect-[${aspect}]`,
      className,
    )}
    style={style}
    {...props}
...
// ── Re-export ResponsiveContainer para no importar de recharts directo ─
